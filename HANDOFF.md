...
**Sung names (2026-09-25).** `lib/sing.py` makes the Piper narrator SING: each word is synthesised, split into syllables at its vowels, and re-pitched and time-stretched onto the melody notes with the WORLD vocoder (pitch lands within ~10 cents; 45 ms glides; light vibrato). `lib/songs.mjs` holds the lyrics+notes (Happy Birthday; the name gets its own notes and any syllable count fits). A `"type": "song"` segment plays the tune once on the music box, then sings it with the child's name (`specs/products/birthday.json` opens with it; `specs/products/teaser.json` is the free 30-second version). `lib/sing_qa.py` is a quality gate: it renders six takes of the name line (whole-word vs syllable-by-syllable × three rates), runs speech recognition on each, and keeps the take where the name is heard best (report next to the cached WAV as `.qa.json`). Setup: `pip install pyworld faster-whisper` (the first QA run downloads Whisper small.en, ~480 MB; `BABYLOOP_QA_MODEL=base.en` is lighter). Honest limits: it sounds like a soft synthetic voice singing, not a trained singer; vowel-first and unusual names are weakest (use `--say`), and Whisper is itself unreliable on singing, so listen to every order before sending.
