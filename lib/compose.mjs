...
export async function composeEpisode(specOrPath, { outDir, preview = false, log = console.log } = {}) {
  ...
  async function buildLoop({ seg, tag, seed }) {
    ...
    return { video, audio, videoEntries, audioEntries, seconds: repeat * loopSec, label: seg.label || scene.name, scene: scene.id, seed, bpm, options, mode, music: music.melody, loopSeconds: loopSec, repeat, ageBand: scene.ageBand, _loopAudio: music };
  }
