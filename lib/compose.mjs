...
export async function composeEpisode(specOrPath, { outDir, preview = false, log = console.log } = {}) {
  ...
  writeConcatList(vlist, pieces.map((s) => ({ path: s.video })));
  writeConcatList(alist, pieces.map((s) => ({ path: s.audio })));
  ...
  async function buildLoop({ seg, tag, seed }) {
    ...
    return { video, audio, seconds: repeat * loopSec, label: seg.label || scene.name, scene: scene.id, seed, bpm, options, music: music.melody, loopSeconds: loopSec, repeat, ageBand: scene.ageBand, _loopAudio: music };
  }
  ...
  async function buildTitle({ seg, tag, seed }) {
    ...
    const pages = [{
      title: seg.text || theme?.title || spec.title, caption: seg.caption || (theme ? theme.name : ''),
