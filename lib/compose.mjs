...
import { speakAll, ensureVoice } from './tts.mjs';
...
export async function composeEpisode(specOrPath, { outDir, preview = false, log = console.log } = {}) {
  ...
  for (let i = 0; i < spec.segments.length; i++) {
    ...
    else if (seg.type === 'song') r = await buildSong({ seg, tag, seed });
  ...
  async function buildLoop({ seg, tag, seed }) {
    ...
    if (says.length) {
      ...
      // evenly spaced lines stay clear of the episode's 4 s tail fade on the last segment
      const clips = placeClips(await speakLines(says), seconds, { tail: seg === spec.segments[spec.segments.length - 1] ? 4.5 : 1.5 });
  ...
  // Sung song: the music box plays the tune once, then the narrator SINGS it
  // (lib/sing.py: Piper syllables re-pitched onto the notes with a WORLD vocoder)
  // with the child's name on its own notes. Picture = a party scene for the whole song.
  async function buildSong({ seg, tag, seed }) {
    const song = SONGS[seg.song || 'happy-birthday'];
    if (!song) throw new Error(`unknown song "${seg.song}"`);
    const bpm = snapBpm(seg.bpm || 96, fps);
    const beat = 60 / bpm;
    const music = renderMusic({ mode: seg.music || 'learn', bpm, melody: song.melody, seed, fps });
    const passS = music.seconds;                       // one pass of the tune, frame-aligned
    const instrumental = seg.instrumental === false ? 0 : 1;
    const nameSung = NAME ? personalizeSpoken('{name}') : song.noName;
    const lines = song.lines.map((l) => ({ text: l.text.replace('{name}', nameSung), notes: l.notes, ...(l.text.includes('{name}') && NAME ? { name: true } : {}) }));
    const voice = await ensureVoice(spec.voice);
    const leadS = 0.35;
    const singSpec = { voice, bpm, transpose: seg.transpose ?? -12, lead: leadS, sampleRate: SR, segments: lines, expect: NAME || '' };
    const key = crypto.createHash('sha1').update(JSON.stringify(singSpec)).digest('hex').slice(0, 16);
    const cacheDir = spec.ttsCacheDir || path.join(outDir, '..', '.tts-cache');
    fs.mkdirSync(cacheDir, { recursive: true });
    const vocalWav = path.join(cacheDir, `sing-${key}.wav`);
    if (!fs.existsSync(vocalWav)) {
      // with a name, sing_qa.py renders several takes and keeps the one where speech recognition hears the name best
      const script = NAME && seg.qa !== false ? 'sing_qa.py' : 'sing.py';
      const r = spawnSync('python3', [path.join(path.dirname(new URL(import.meta.url).pathname), script)], { input: JSON.stringify({ ...singSpec, out: vocalWav }), encoding: 'utf8', maxBuffer: 1 << 24 });
      if (r.status !== 0) throw new Error(`${script} failed: ${(r.stderr || '').slice(-600)}`);
      try { const q = JSON.parse(r.stdout).qa; if (q) { fs.writeFileSync(vocalWav + '.qa.json', JSON.stringify(q, null, 2)); log(`  sung name QA: ${q.chosen && JSON.stringify(q.chosen)} · heard "${q.heard || ''}" · score ${q.score ?? '—'}`); } } catch { /* QA report is optional */ }
    }
    const vocal = loadClip(vocalWav);
    const vocalStart = instrumental * passS - leadS;   // first syllable lands on beat 1 of the sung pass
    const total = roundUp(instrumental * passS + passS + Math.max(1.5, beat * 2));
    const bed = tileBed(music, total);
    // music at full level for the instrumental pass, dipped under the voice, voice on top
    const dip = seg.bedUnderVoice ?? 0.55, rampN = Math.round(0.4 * SR), v0 = Math.max(0, Math.round(vocalStart * SR));
    for (let i = 0; i < bed.n; i++) {
      const g = i < v0 - rampN ? 1 : i < v0 ? 1 - (1 - dip) * ((i - (v0 - rampN)) / rampN) : dip;
      bed.L[i] *= g; bed.R[i] *= g;
      const j = i - v0;
      if (j >= 0 && j < vocal.length) { const x = vocal[j] * (seg.voiceGain ?? 0.95); bed.L[i] += x; bed.R[i] += x; }
    }
    const audio = path.join(work, `${tag}.wav`);
    fs.writeFileSync(audio, encodeWav(bed.L, bed.R));
    const scene = getScene(seg.scene || 'dancing-fruits');
    const video = path.join(work, `${tag}.mp4`);
    const options = { ...(seg.options || {}), palette: seg.palette || scene.defaults.palette, theme: seg.theme || spec.theme || null };
    await renderVideo({ scene, seconds: total, out: video, fps, width, height, seed, bpm, options, ...enc });
    return { video, audio, seconds: total, label: personalize(seg.label || (NAME ? `${song.title}, ${NAME}` : song.title)), scene: scene.id, seed, bpm, music: music.melody, sung: true, narrated: true, ageBand: scene.ageBand };
  }
  ...
  async function buildStory({ seg, tag, seed }) {
    ...
    // `after` = quiet time kept after the last line (4.5 s on the end card so the
    // goodnight finishes before the episode's 4 s fade-out).
    const sequence = (list, from, minSec, after = 1.5) => {
      let tt = from;
      for (const c of list) if (c.at === undefined) { c.at = tt; tt += c.seconds + 0.5; }
      return Math.max(minSec, roundUp(list.length ? tt - 0.5 + after : 0));
    };
    ...
    const endSec = sequence(outro, lead, roundUp(preview ? 2.5 : 5), 4.5);
