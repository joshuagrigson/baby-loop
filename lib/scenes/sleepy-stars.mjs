...
import { vgradient, circle, face, cloud, emoji, label, star, shaded, vignette } from '../draw.mjs';
...
export default {
  ...
  draw(ctx, t, s, { W, H, phase, loopSeconds }) {
    ...
    const glow = ctx.createRadialGradient(mx, my, mr * 0.9, mx, my, mr * 2.4);
    glow.addColorStop(0, withAlpha(pal.moon, 0.28)); glow.addColorStop(0.5, withAlpha(pal.moon, 0.08)); glow.addColorStop(1, withAlpha(pal.moon, 0));
    ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(mx, my, mr * 2.4, 0, TAU); ctx.fill();
    circle(ctx, mx, my, mr, shaded(ctx, pal.moon, mr, mx, my, 0.7));
    ...
    // sleepy friends: an owl dozing on the fence, and three animals that each
    // cross once per loop and hop the fence in the middle
    emoji(ctx, '🦉', fx + W * 0.06, fy - H * 0.12 + Math.sin(TAU * phase * 2) * H * 0.004, H * 0.09);
    for (const a of [{ e: '🐑', off: 0 }, { e: '🐇', off: 0.37 }, { e: '🐢', off: 0.7 }]) {
