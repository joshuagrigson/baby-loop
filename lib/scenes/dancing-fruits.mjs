...
export default {
  ...
  init({ W, H, rng, loopSeconds, options = {} }) {
    ...
    // Portrait (9:16 clips): the frame is too narrow for a row of five, so the
    // troupe stacks in three shorter rows and sizes follow the short side (W),
    // which keeps characters the same pixel size as in landscape. Landscape is
    // untouched: unit === H there.
    const portrait = H > W;
    const unit = Math.min(W, H);
    const front = Math.min(count, portrait ? 3 : 5);
    const back = Math.min(portrait ? 3 : 4, Math.max(0, count - front));
    const far = portrait ? Math.min(2, Math.max(0, count - front - back)) : 0;
    ...
    const mk = (kind, i, n, row) => {
      ...
      const spacing = W / (n + 1);
      const base = portrait ? [0.3, 0.24, 0.19][row] : row === 0 ? (n >= 5 ? 0.3 : n === 4 ? 0.34 : 0.4) : 0.17;
      return {
        kind, row,
        x: spacing * (i + 1),
        size: unit * base * rng.range(0.92, 1.08),
    ...
    const chars = [
      ...Array.from({ length: far }, (_, i) => mk(kinds[front + back + i], i, far, 2)),
    ...
    const floors = portrait ? [H * 0.84, H * 0.64, H * 0.48] : [H * 0.8, H * 0.44];
    return { pal, chars, dot, notes, pieces, loopSeconds, floors, unit };
  },
  ...
  draw(ctx, t, s, { W, H, beat, phase, loopSeconds, bpm }) {
    ...
    const floors = s.floors || [H * 0.8, H * 0.44];
    const unit = s.unit || H;
    for (const c of s.chars) {
      ...
      const height = hop * unit * (c.row ? 0.05 : 0.09) * (hopEvery === 2 ? 1.6 : 1);
