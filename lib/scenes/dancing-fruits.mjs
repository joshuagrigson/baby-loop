...
import { fruit, FRUITS, circle, shadow, label, confetti, makeConfetti } from '../draw.mjs';
...
export default {
  ...
  draw(ctx, t, s, { W, H, beat, phase, loopSeconds }) {
    ...
    for (const c of s.chars) {
      ...
      const height = hop * H * (c.row ? 0.05 : 0.09) * (c.hopEvery === 2 ? 1.6 : 1);
      ...
      const mouth = 0.3 + 0.55 * pulse(beat / 2 + c.beatOffset);   // big happy grin, opens on the beat
      ...
      fruit(ctx, c.kind, c.x, floor - c.size * 0.5 - height, c.size, { squash, tilt, blink: bl, mouth, look, hat: c.hat });
    }
  },
};
