...
import { drawBackdrop, captionPill, shadow, label, sparkle, offscreenCanvas } from '../draw.mjs';
...
function drawPage(ctx, page, local, info) {
  ...
  // Portrait (9:16): props scale with the short side so the tableau fits the
  // narrow page, and the title drops below the backdrop's sun / moon instead
  // of colliding with it. Landscape is unchanged (unit === H).
  const portrait = H > W;
  const unit = Math.min(W, H);
  const size = unit * PROP_SIZE[Math.min(n, PROP_SIZE.length - 1)];
  ...
  if (page.title) {
    ...
    ctx.save();
    ctx.translate(W / 2, portrait ? H * 0.4 : H * 0.17);
    ctx.scale(pop, pop);
    label(ctx, page.title, 0, 0, { size: unit * 0.13, gradient: ['#ffffff', '#ffe08a'], stroke: '#2b2b3a', lw: unit * 0.02, maxWidth: W * (portrait ? 0.86 : 0.72), shadow: unit * 0.025 });
    ctx.restore();
  }
...
// The frame never changes, so bake it once per size/colour and blit the four
// border strips each frame.
const FRAMES = new Map();
function drawFrame(ctx, W, H, color) {
  const key = `${W}x${H}|${color}`;
  let f = FRAMES.get(key);
  if (f === undefined) {
    f = offscreenCanvas(ctx, W, H);
    if (f) bookFrame(f.getContext('2d'), W, H, color);
    if (FRAMES.size > 4) FRAMES.delete(FRAMES.keys().next().value);
    FRAMES.set(key, f);
  }
  if (!f) return bookFrame(ctx, W, H, color);
  const T = Math.ceil(H * 0.1);
  ctx.drawImage(f, 0, 0, W, T, 0, 0, W, T);
  ctx.drawImage(f, 0, H - T, W, T, 0, H - T, W, T);
  ctx.drawImage(f, 0, T, T, H - 2 * T, 0, T, T, H - 2 * T);
  ctx.drawImage(f, W - T, T, T, H - 2 * T, W - T, T, T, H - 2 * T);
}
