...
import { drawBackdrop, circle, ellipse, face, emoji, confetti, makeConfetti } from '../draw.mjs';
  ...
  draw(ctx, t, s, { W, H, beat, phase, loopSeconds }) {
    ...
    for (const f of s.flowers) {
      ...
      for (let p = 0; p < f.petals; p++) {
        ...
        ellipse(ctx, Math.cos(a) * f.r * 1.05 * bloom, Math.sin(a) * f.r * 1.05 * bloom, f.r * 0.62 * bloom, f.r * 0.4 * bloom, f.color, a);
      }
      circle(ctx, 0, 0, f.r * 0.78, f.centre, '#1b1b1b', H * 0.004);
