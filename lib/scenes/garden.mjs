...
export default {
  ...
  init({ W, H, rng, loopSeconds = 30, options = {} }) {
    ...
    const flyers = [
      { kind: 'bee', y: H * 0.3, k: 1, off: 0.0, amp: H * 0.05, loops: 3, loopR: H * 0.07, size: H * 0.11, flapK: flap(5) },
      { kind: 'butterfly', y: H * 0.2, k: 1, off: 0.5, amp: H * 0.07, loops: 2, loopR: H * 0.05, size: H * 0.13, dir: -1, flapK: flap(1.4), color: '#ff7eb6' },
      { kind: 'butterfly', y: H * 0.42, k: 1, off: 0.2, amp: H * 0.05, loops: 2, loopR: H * 0.04, size: H * 0.1, flapK: flap(1.7), color: '#7cc7ff', color2: '#fff3a6' },
      { kind: 'bee', y: H * 0.46, k: 1, off: 0.72, amp: H * 0.04, loops: 3, loopR: H * 0.05, size: H * 0.09, dir: -1, flapK: flap(5.5) },
    ...
    return { pal, flowers, flyers, trees, blades, specks, pieces, hat };
  },
  ...
  draw(ctx, t, s, { W, H, phase, loopSeconds, bpm }) {
    ...
    const sx = W * 0.84, sy = H * 0.17, sr = H * 0.075;
    ...
    // ladybug strolls along the grass once per loop
    {
      ...
      critter(ctx, 'ladybug', lx, H * 0.925 - step * H * 0.006, H * 0.095, { tilt: (step - 0.5) * 0.08, blink: blink(t, loopSeconds, 3.7, 0.4), mouth: 0.4, hat: s.hat });
    }
