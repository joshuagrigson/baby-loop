...
// Pattern sizes follow the short side of the frame, so a 9:16 clip shows the
// same bullseye / face / cells as 16:9 instead of cropping them. In landscape
// S === H, so nothing changes there.
const short = (W, H) => Math.min(W, H);
...
const patterns = [
  ...
  (ctx, u, { W, H }) => {
    ...
    const S = short(W, H);
    const R = reach(W, H), k = S * 0.18, arms = 4;
    ctx.save();
    ...
    ctx.fill();
    ctx.restore();
    circle(ctx, W / 2, H / 2, S * 0.065, RED);
  },
  // 2. expanding rings
  (ctx, u, { W, H }) => {
    ctx.fillStyle = BLACK; ctx.fillRect(0, 0, W, H);
    const S = short(W, H);
    const gap = S * 0.11, R = reach(W, H);
  ...
  (ctx, u, { W, H }) => {
    const n = 6;
    const S = short(W, H);
    const cell = (S * (1.0 + 0.12 * Math.sin(u * TAU))) / n;
  ...
  (ctx, u, { W, H }) => {
    ctx.fillStyle = WHITE; ctx.fillRect(0, 0, W, H);
    const S = short(W, H);
    for (let i = 6; i >= 1; i--) circle(ctx, W / 2, H / 2, S * 0.075 * i, i % 2 === 0 ? WHITE : BLACK);
    const a = u * TAU * 2;
    const ox = W / 2 + Math.cos(a) * S * 0.38, oy = H / 2 + Math.sin(a) * S * 0.38;
    circle(ctx, ox, oy, S * 0.062, BLACK);
    circle(ctx, ox, oy, S * 0.05, RED);
  },
  ...
  (ctx, u, { W, H }) => {
    ...
    ctx.fill();
    ctx.restore();
    circle(ctx, W / 2, H / 2, S * 0.19, WHITE);
    circle(ctx, W / 2, H / 2, S * 0.19, null, BLACK, S * 0.012);
    star(ctx, W / 2, H / 2 + S * 0.01, S * 0.14, S * 0.06, 5, RED, BLACK, S * 0.012);
  },
  ...
  (ctx, u, { W, H, t, loopSeconds }) => {
    ...
    heart(ctx, W / 2, H / 2 + s * 0.5, s * 0.16, RED, WHITE, S * 0.008);
  },
];
