...
const PASSENGERS = ['bear', 'bunny', 'panda', 'frog', 'lion', 'koala', 'pig', 'fox', 'cow', 'monkey', 'cat', 'dog', 'elephant', 'duck', 'chick'];
const INK = '#2a2230';
...
export default {
  ...
  init({ W, H, rng, options = {} }) {
    ...
    return { pal, hat, cars, driver, items, fg, pieces, engine: pal.theme ? pal.bg : '#e63946', sky: pal.theme ? pal.bgs[0] : '#6ec6ff' };
  },
  ...
  draw(ctx, t, s, { W, H, beat, phase, loopSeconds }) {
    ...
    s.cars.forEach((c, i) => {
      ...
      c.riders.forEach((r, j) => {
        ...
        critter(ctx, r.kind, rx, ry, size, {
          squash: 0.25 * (1 - hop) - 0.1 * hop, tilt: Math.sin(Math.PI * (beat / 2 + r.off)) * 0.1,
          blink: blink(t, loopSeconds, 3.3, r.blinkOffset), mouth: 0.3 + 0.55 * pulse(beat / 2 + r.off),
          look: [0.5 + 0.4 * Math.sin(TAU * phase * 2 + r.lookPhase), 0.1], brow: 0.35 + 0.6 * hop,
          wave: 0.25 + 0.75 * Math.sin(Math.PI * (beat / 2 + r.off)), hat: s.hat,
        });
      });
    ...
    critter(ctx, s.driver, wx + ww * 0.5, wy + wh * 0.62 - bounceAt(0.5) * 0.6, wh * 1.15, {
      blink: blink(t, loopSeconds, 3.7, 1.1), mouth: 0.25 + 0.5 * pulse(beat / 2 + 0.5), look: [0.8, 0.15], brow: 0.6, wave: -0.6,
      hat: s.hat !== 'none' ? s.hat : null, tilt: Math.sin(Math.PI * beat / 2) * 0.06,
    });
    ...
    ctx.beginPath(); ctx.ellipse(fcx + u * 0.03, fcy, fr * 0.7, fr * 0.9, 0, 0, TAU); outlined(ctx, lit(ctx, '#f4efe8', fcx, fcy, fr * 0.9), lw);
    face(ctx, { x: fcx + u * 0.05, y: fcy + u * 0.02, size: fr * 1.55, blink: blink(t, loopSeconds, 3, 0.7), mouth: 0.25 + 0.55 * pulse(beat / 2), look: [0.35, 0.1], brow: 0.4 + 0.4 * pulse(beat), cheeks: true, eyeSpacing: 0.21, eyeScale: 1.12 });
    // lamp
