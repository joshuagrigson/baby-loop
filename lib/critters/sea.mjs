...
import { drawCritter, TAU, withAlpha } from './core.mjs';
...
export const fish = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  ...
  ctx.restore();
  part(k, () => { ctx.moveTo(-s * 0.26, -s * 0.2); ctx.bezierCurveTo(-s * 0.3, -s * 0.44, -s * 0.12, -s * 0.56, 0, -s * 0.5); ctx.quadraticCurveTo(-s * 0.02, -s * 0.44, s * 0.1, -s * 0.36); ctx.closePath(); },
  ...
  part(k, () => ctx.ellipse(s * 0.06, 0, s * 0.42, s * 0.38, 0, 0, TAU), c, s * 0.44, s * 0.06, 0, () => {
    ...
    ctx.beginPath(); ctx.ellipse(-s * 0.46, 0, s * 0.28, s * 0.5, 0, 0, TAU); ctx.ellipse(-s * 0.46, 0, s * 0.2, s * 0.48, 0, 0, TAU, true); ctx.fillStyle = st; ctx.fill('evenodd'); k.stroke(k.lw * 0.7);
    gloss(k, -s * 0.04, -s * 0.26, s * 0.16, s * 0.05, -0.15, 0.4);
  });
  return { x: s * 0.12, y: -s * 0.04, size: s * 0.7, hatSize: s * 0.55 };
});

export const dolphin = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  const { s, light, dark } = k;
  ...
  ctx.restore();
  return { x: s * 0.17, y: -s * 0.1, size: s * 0.56, eyeScale: 1.1, hatSize: s * 0.46 };
});

export const octopus = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  const { s, light, dark } = k;
  ...
  return { x: s * 0.03, y: -s * 0.1, size: s * 0.7, hatSize: s * 0.62 };
});

export const crab = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  ...
  return { y: s * 0.1, size: s * 0.66, eyeY: -0.62, eyeSpacing: 0.22, brows: false, mouthY: 0.2, hatSize: s * 0.5 };
});

export const turtle = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  ...
  return { x: s * 0.29, y: -s * 0.17, size: s * 0.56, hatSize: s * 0.46 };
});

export const whale = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  const { s, light, dark } = k;
  ...
  ctx.restore();
  return { x: s * 0.22, y: -s * 0.06, size: s * 0.62, hatSize: s * 0.5 };
});

export const seahorse = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  const { s, light, dark } = k;
  ...
  tube(k, [[s * 0.14, -s * 0.2], [s * 0.3, -s * 0.18], [s * 0.4, -s * 0.16]], s * 0.15, s * 0.08, c);  // snout
  k.blob(s * 0.42, -s * 0.16, s * 0.035, s * 0.055, light(c, 0.3));
  k.blob(0, -s * 0.24, s * 0.27, s * 0.26, c);                                             // head
  gloss(k, -s * 0.1, -s * 0.4, s * 0.08, s * 0.035, -0.4, 0.4);
  return { x: s * 0.02, y: -s * 0.23, size: s * 0.54, brows: false, hatSize: s * 0.46 };
});

export const jellyfish = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  ...
  for (const sx of [-1, 1]) {                                                              // frilly oral arms
    const pts = []; for (let j = 0; j <= 10; j++) { const u = j / 10; pts.push([s * (sx * 0.07 + Math.sin(u * 7 + w + sx) * 0.05), s * (0.0 + u * 0.36)]); }
    ctx.globalAlpha = 0.75; tube(k, pts, s * 0.1, s * 0.03, lc); ctx.globalAlpha = 1;
  }
  const bell = () => {
  ...
  ctx.globalAlpha = 1;
  return { x: s * 0.02, y: -s * 0.17, size: s * 0.64, hatSize: s * 0.56 };
});

export const pufferfish = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  const { s, light, dark } = k;
  ...
  ctx.restore();
  return { x: s * 0.07, y: -s * 0.02, size: s * 0.72, hatSize: s * 0.6 };
});

export const starfish = (ctx, x, y, size, o = {}) => drawCritter(ctx, x, y, size, o, (k) => {
  const { s, light } = k;
  ...
  return { x: 0, y: s * 0.06, size: s * 0.6, hatSize: s * 0.48 };
});
