...
export function drawCritter(ctx, x, y, size, o, draw) {
  ...
  const k = kit(ctx, s, o);
  const f = draw(k);
  if (f) { const { hat: _h, hatSize: _hs, ...fo } = f; face(ctx, { x: 0, y: 0, size: s * 0.8, blink, mouth, look, brow, ink: o.ink || '#1b1b1b', ...fo }); }
  if (o.hat && f !== false) {
    // f.hat = [x, headTop, size] places the hat on an off-centre head;
    // otherwise it sits on the body centre, sized by f.hatSize.
    if (f && Array.isArray(f.hat)) {
      const [hx, top, hs] = f.hat;
      ctx.save(); ctx.translate(hx, top + hs * 0.5); accessory(ctx, o.hat, hs, k.outline); ctx.restore();
    } else accessory(ctx, o.hat, (f && f.hatSize) || s * 0.8, k.outline);
  }
  ctx.restore();
}
