...
function drawCaption(ctx, text, W, H, { pop = 1, alpha = 1, unit }) {
  ...
  // Honour the spec's own line breaks: shrink to fit them first, and only
  // re-wrap by words if the text is still too wide at the smallest size.
  const maxW = W * 0.84;
  const given = String(text).split('\n').filter((l) => l.trim().length);
  let size = unit * 0.095;
  const widestGiven = () => { ctx.font = `700 ${size}px ${FONT_DISPLAY}`; return Math.max(...given.map((l) => ctx.measureText(l).width)); };
  while (widestGiven() > maxW && size > unit * 0.07) size *= 0.94;
  let lines = widestGiven() <= maxW ? given : wrapLines(ctx, text, size, maxW);
  for (let i = 0; i < 6 && lines.length > 3; i++) { size *= 0.9; lines = wrapLines(ctx, text, size, maxW); }
  const lh = size * 1.12;
...
export async function composeClip(specOrPath, { outDir, preview = false, log = console.log } = {}) {
  ...
  await renderVideo({ scene: wrapper, seconds: total, out: video, fps, width, height, seed, bpm, ...enc, onProgress: (i, n, s) => { if ((i - 1) % (fps * 10) === 0 || i === n) log(`  ${String(i).padStart(5)}/${n} frames  ${(i / s).toFixed(1)} fps`); } });
