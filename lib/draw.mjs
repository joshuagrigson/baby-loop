...
export function atmosphere(ctx, W, H, { bg, pops = [], phase = 0, bokeh = 14, vignette = 0.22, rng = null } = {}) {
  ...
  for (let i = 0; i < bokeh; i++) {
    ...
    const k = 1 + (i % 2);
    const x = (fx + Math.sin(TAU * phase * k + i) * 0.03) * W;
    const y = (fy + Math.cos(TAU * phase * k + i * 1.7) * 0.04) * H;
...
export function drawBackdrop(ctx, name, W, H, { phase = 0, t = 0 } = {}) {
  ...
  // static sky and ground layers are baked once per (backdrop, size); only the
  // moving parts (sun, stars, clouds, flags, waves) are drawn every frame
  const layers = backdropLayers(ctx, name, bd, W, H, horizon);
  if (layers) ctx.drawImage(layers.sky, 0, 0, W, H);
  else backdropSky(ctx, bd, W, H, horizon);
  if (bd.stars) nightSky(ctx, W, H, horizon, phase);
  ...
  if (layers) ctx.drawImage(layers.ground, 0, 0, W, H);
  else backdropGround(ctx, name, bd, W, H, horizon);
  if (bd.castle) castleFlags(ctx, W, H, horizon, phase);
  if (bd.waves) {
    for (let k = 0; k < 3; k++) {
...
// Scenes can bake their own static layers with this (null if unavailable).
export function offscreenCanvas(ctx, w, h) { return offscreen(ctx, w, h); }

// --- Stickers ------------------------------------------------------------------
const STICKERS = new Map();
const STICKER_MAX = 240;                 // entries
const STICKER_BUDGET = 96 * 1024 * 1024; // bytes of baked pixels
let stickerBytes = 0;
...
function buildSticker(ctx, char, q) {
  ...
  for (const [rr, n] of [[lw, 36], [lw * 0.7, 24], [lw * 0.4, 12]]) {
...
function drawSticker(ctx, char, x, y, size, alpha = 1) {
  ...
  if (st === undefined) {
    ...
    const bytes = st ? st.S * st.S * 4 : 0;
    while (STICKERS.size && (STICKERS.size >= STICKER_MAX || stickerBytes + bytes > STICKER_BUDGET)) {
      const [k, old] = STICKERS.entries().next().value;
      STICKERS.delete(k);
      stickerBytes -= old ? old.S * old.S * 4 : 0;
    }
    STICKERS.set(key, st);
    stickerBytes += bytes;
  } else {
    STICKERS.delete(key); STICKERS.set(key, st);   // LRU touch
  }
  if (!st) return false;
  const d = st.S * (size / q);
  ctx.save();
...
function confettiPiece(ctx, p, phase) {
  ...
  switch (p.shape) {
    case 'ribbon': {
      // flat paper streamer with a gentle S twist
      ctx.beginPath();
      ctx.moveTo(-s * 0.6, s * 0.05);
      ctx.bezierCurveTo(-s * 0.25, -s * 0.28, s * 0.15, s * 0.28, s * 0.6, -s * 0.05);
      ctx.lineCap = 'butt';
      ctx.strokeStyle = col; ctx.lineWidth = s * 0.2; ctx.stroke();
      ctx.strokeStyle = withAlpha(hi, f >= 0 ? 0.45 : 0.12); ctx.lineWidth = s * 0.06; ctx.stroke();
      break;
    }
    case 'curl': {
      ctx.beginPath();
...
export function sunshine(ctx, x, y, r, { phase = 0, face: withFace = true, rays = 12, blink: bl = 0 } = {}) {
  ...
  ctx.restore();
  circle(ctx, x, y, r * 1.05, '#ffae2b');
  const sg = ctx.createRadialGradient(x - r * 0.35, y - r * 0.4, r * 0.05, x, y, r * 1.05);
  sg.addColorStop(0, '#fff7c2'); sg.addColorStop(0.5, '#ffdc4e'); sg.addColorStop(1, '#ffbe2e');
  circle(ctx, x, y, r, sg);
...
export function softCloud(ctx, x, y, w, { body = '#ffffff', shade = '#b9cde6', alpha = 1 } = {}) {
  ...
  const puff = (dy, sc) => {
    ctx.beginPath();
    // separate sub-paths (moveTo) so no connecting slivers get filled
    for (const [ox, oy, rx, ry] of [[0, 0, 0.5, 0.42], [-0.24, 0.08, 0.26, 0.4], [0.24, 0.06, 0.28, 0.44], [-0.06, -0.3, 0.24, 0.48], [0.14, -0.22, 0.2, 0.4]]) {
      const cx = x + w * ox, cy = y + dy + h * oy;
      ctx.moveTo(cx + w * rx * sc, cy);
      ctx.ellipse(cx, cy, w * rx * sc, h * ry * sc, 0, 0, TAU);
    }
  };
  ctx.save();
  ctx.globalAlpha *= alpha;
...
function storyCastle(ctx, W, H, horizon) {
  const wall = '#efe2f7', wallDark = '#c7b3dc', roof = '#e05a7a';
  ...
  // door
  roundRect(ctx, W / 2 - W * 0.035, horizon - H * 0.2, W * 0.07, H * 0.16, W * 0.035, '#8a5d34');
  ctx.strokeStyle = withAlpha('#4a2e14', 0.5); ctx.lineWidth = H * 0.003;
  ctx.beginPath(); ctx.moveTo(W / 2, horizon - H * 0.19); ctx.lineTo(W / 2, horizon - H * 0.04); ctx.stroke();
}

// waving pennants on the castle towers (live, periodic)
function castleFlags(ctx, W, H, horizon, phase) {
  const bw = W * 0.46, bx = W / 2 - bw / 2, by = horizon - H * 0.26;
  for (const cx of [bx, bx + bw]) {
    const fy = by - H * 0.14 - H * 0.15;
    ctx.strokeStyle = '#6b4a2b'; ctx.lineWidth = H * 0.004; ctx.lineCap = 'round';
...
// --- Backdrop layer cache ------------------------------------------------------------
const BACKDROP_LAYERS = new Map();
const BACKDROP_MAX = 6;

function backdropLayers(ctx, name, bd, W, H, horizon) {
  const key = `${name}|${W}x${H}`;
  let L = BACKDROP_LAYERS.get(key);
  if (L === undefined) {
    const sky = offscreen(ctx, W, H), ground = offscreen(ctx, W, H);
    if (sky && ground) {
      backdropSky(sky.getContext('2d'), bd, W, H, horizon);
      backdropGround(ground.getContext('2d'), name, bd, W, H, horizon);
      L = { sky, ground };
    } else L = null;
    if (BACKDROP_LAYERS.size >= BACKDROP_MAX) BACKDROP_LAYERS.delete(BACKDROP_LAYERS.keys().next().value);
    BACKDROP_LAYERS.set(key, L);
  }
  return L;
}

function backdropSky(ctx, bd, W, H, horizon) {
  ...
  ctx.fillStyle = sky;
  ctx.fillRect(0, 0, W, H);
  // light pooling near the horizon
...
// everything from the far hills forward (transparent above), static
function backdropGround(ctx, name, bd, W, H, horizon) {
  ...
  if (bd.castle) storyCastle(ctx, W, H, horizon);
  if (bd.trees) storyTrees(ctx, W, H, horizon);
  if (name === 'cottage') storyFence(ctx, W, H, horizon);
  if (bd.ground) {
    const hill = (cx, cy, rx, ry, col) => {
      // rim light: a paler copy peeking above the body (top of the hill only)
      ctx.save();
      ctx.beginPath(); ctx.rect(0, 0, W, horizon + H * 0.06); ctx.clip();
      ellipse(ctx, cx, cy - H * 0.012, rx, ry, mixHex(col, '#ffffff', 0.35));
      ctx.restore();
      const g = ctx.createLinearGradient(0, cy - ry, 0, H);
      g.addColorStop(0, mixHex(col, '#ffffff', 0.1)); g.addColorStop(1, mixHex(col, '#000000', 0.14));
      ellipse(ctx, cx, cy, rx, ry, g);
    };
    // two overlapping hills; together they cover the whole foreground
    hill(W * 0.85, horizon + H * 0.3, W * 0.58, H * 0.32, bd.ground2);
    hill(W * 0.28, horizon + H * 0.22, W * 0.64, H * 0.3, bd.ground);
    groundDetail(ctx, W, H, horizon, bd, 0);
  }
}
