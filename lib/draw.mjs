...
export function face(ctx, o) {
  ...
  if (mouth < 0.15) {
    ...
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.arc(x, my - size * 0.05, size * 0.12, 0.15 * Math.PI, 0.85 * Math.PI);
    ctx.stroke();
  } else {
    const mw = size * 0.13 * (0.7 + 0.3 * mouth), mh = size * 0.15 * mouth;
    ctx.save();
    ctx.beginPath();
    ...
    ctx.fill();
    ctx.clip();
    ellipse(ctx, x, my + mh * 0.9, mw * 0.8, mh * 0.7, '#ff7b9c'); // tongue
    ctx.restore();
    ctx.strokeStyle = ink;
...
// --- Fruit characters -------------------------------------------------------
export const FRUITS = ['strawberry', 'orange', 'blueberry', 'pear', 'lemon', 'watermelon', 'apple', 'grape'];
