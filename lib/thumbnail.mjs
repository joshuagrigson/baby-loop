...
const HEROES = {
  ...
  balloons: ['critter:bear', 'critter:bunny', 'critter:panda'],
  ...
  train: ['critter:bear', 'critter:lion', 'critter:panda'],
...
export function renderThumbnail({ spec, segments = [], out, width = 1280, height = 720, totalSeconds = 0 }) {
  ...
  const heroes = (spec.thumbnailHeroes || HEROES[scene.id] || []).filter(available).slice(0, 3);
  ...
  if (useHeroes) {
    ...
    order.forEach((tok, i) => {
      ...
      sticker(ctx, width, height, height * 0.012, (lx) => {
        ...
        drawProp(lx, tok, 0, 0, height * L.s, { mouth: 0.75, wave: 0.8, brow: 1, blink: 0, flip: L.x > 0.6 && tok.startsWith('critter') });
        lx.restore();
      });
    });
  }
