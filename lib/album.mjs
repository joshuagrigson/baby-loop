...
export function credit(melodyId) {
  ...
  return { ...c, line: `${short}, arr. ${ARTIST}`, full: `${c.composer}${c.year ? ` (${c.year})` : ''}, arr. ${ARTIST}` };
}
...
export function drawCover(ctx, S, spec) {
  ...
  // moon (dozing)
  const mx = S * 0.78, my = S * 0.25, mr = S * 0.105;
  ...
  // clouds
  softCloud(ctx, S * 0.16, S * 0.50, S * 0.26, { rim: '#aebcee', top: '#4d5f99', belly: '#2d3d72', rimW: 0.04 });
  softCloud(ctx, S * 0.62, S * 0.56, S * 0.2, { rim: '#aebcee', top: '#4d5f99', belly: '#2d3d72', rimW: 0.04, alpha: 0.9 });
  softCloud(ctx, S * 0.94, S * 0.47, S * 0.22, { rim: '#aebcee', top: '#4d5f99', belly: '#2d3d72', rimW: 0.04, alpha: 0.85 });
  // hills
  ...
  for (let i = 0; i < 3; i++) label(ctx, 'z', S * (0.64 + i * 0.045), ground - hs * (0.78 + i * 0.13), { size: S * (0.035 + 0.014 * i), fill: c.star, stroke: null, alpha: 0.9 - i * 0.2 });
  // title
  ...
  const by = S * 0.06 + lines.length * big * 1.05 + S * 0.02;
  if (badge) {
    const bw = S * 0.2, bh = S * 0.065;
    roundRect(ctx, S * 0.5 - bw / 2, by, bw, bh, bh / 2, withAlpha('#0c1538', 0.55), withAlpha(c.star, 0.8), S * 0.004);
    label(ctx, badge, S * 0.5, by + bh / 2 + S * 0.002, { size: bh * 0.6, fill: c.star, stroke: null });
  }
  // artist
...
export async function renderAlbum(specOrPath, { outDir = null, mp3Only = false, only = null, sleep = null, onProgress = null } = {}) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const meta = {
      ...
      comment: `${cr.full}. Public-domain composition; original recording by ${artist}.`,
    };
    const files = { wav };
    ...
    const info = {
      ...
      bpm: r.bpm, key: r.key, seed: r.seed, seconds: +r.seconds.toFixed(2), duration: fmt(r.seconds), structure: r.structure.map((s) => s.kind).join(' '),
...
export function bandcampText(a) {
  ...
  return [
    ...
    ...a.tracks.map((t) => `${String(t.n).padStart(2, '0')}. ${t.title}\n    ${t.work}${t.composed ? ` (${t.composed})` : ''}. ${t.creditFull}. Original recording ℗ ${a.year} ${a.artist}. Public-domain composition.`),
    ``,
  ].join('\n');
}
